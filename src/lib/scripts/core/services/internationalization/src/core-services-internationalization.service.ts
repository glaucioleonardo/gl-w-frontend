...
class Core {
  retrieveList(main?: boolean, countryName?: TCountryName, currencyFullName?: TCurrentFullName, currencyCode?: TCurrencyCode, countryLanguage?: TCountryLanguage): IInternationalizationList[] {
    let list: IInternationalizationList[] =  [
      {
    ...
    if (main != null) { list = list.filter(x => x.main === main); }
    if (countryName != null) { list = list.filter(x => x.countryName === countryName); }
    if (currencyFullName != null) { list = list.filter(x => x.currencyFullName === currencyFullName); }
    if (currencyCode != null) { list = list.filter(x => x.currencyCode === currencyCode); }
    if (countryLanguage != null) { list = list.filter(x => x.countryLanguage === countryLanguage); }
