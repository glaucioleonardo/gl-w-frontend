class Core {
    retrieveList(main, countryName, currencyFullName, currencyCode, countryLanguage) {
        let list = [
            {
        ...
        if (main != null) {
            list = list.filter(x => x.main === main);
        }
        if (countryName != null) {
            list = list.filter(x => x.countryName === countryName);
        }
        if (currencyFullName != null) {
            list = list.filter(x => x.currencyFullName === currencyFullName);
        }
        if (currencyCode != null) {
            list = list.filter(x => x.currencyCode === currencyCode);
        }
        if (countryLanguage != null) {
            list = list.filter(x => x.countryLanguage === countryLanguage);
        }
        return list;
    }
}
